...
public class AirtimeServiceImpl implements AirtimeService {
    ...
    @Override
    public PurchaseAirtimeResponse purchaseAirtime(PurchaseAirtimeRequest purchaseAirtimeRequest) throws InvalidRequestException, IOException {
        validatePurchaseRequest(purchaseAirtimeRequest);
        return getPurchaseAirtimeResponse(purchaseAirtimeRequest);
    }

    private PurchaseAirtimeResponse getPurchaseAirtimeResponse(PurchaseAirtimeRequest purchaseAirtimeRequest) throws InvalidRequestException, IOException {
        ...
        if (!response.isSuccessful()) {
            throw new InvalidRequestException(responseBodyAsJson.getString("responseMessage"));
        }
    ...
    @Override
    public AirtimeStatusDto findByRequestId(String requestId) throws NotFoundException {
        Airtime airtime = airtimeRepository.findByRequestId(requestId).orElseThrow(() -> new NotFoundException("Airtime not Found"));
        return AirtimeStatusDto.builder()
                .id(airtime.getId())
                .userId(airtime.getUser().getId().toString())
                .requestId(airtime.getRequestId())
                .responseCode(airtime.getResponseCode())
                .responseMessage(airtime.getResponseMessage())
                .build();
    }
